...
job "signoz" {
  ...
  group "infrastructure" {
    ...
    # Init task to download histogram binary
...
cp histogram-quantile /alloc/data/user_scripts/histogramQuantile
EOF
        ]
  ...
  group "signoz-app" {
    ...
    task "schema-migrator-sync" {
      ...
      config {
        ...
        command = "sync"
        args = [
          "--dsn=tcp://${NOMAD_UPSTREAM_ADDR_clickhouse}",
          "--up="
        ]
      }
    ...
    task "signoz" {
      ...
      env {
        ...
        SIGNOZ_TELEMETRYSTORE_CLICKHOUSE_DSN = "tcp://${NOMAD_UPSTREAM_ADDR_clickhouse}"
...
exporters:
  clickhousetraces:
    datasource: tcp://${NOMAD_UPSTREAM_ADDR_clickhouse}/signoz_traces
    low_cardinal_exception_grouping: ${LOW_CARDINAL_EXCEPTION_GROUPING}
    use_new_schema: true
  signozclickhousemetrics:
    dsn: tcp://${NOMAD_UPSTREAM_ADDR_clickhouse}/signoz_metrics
  clickhouselogsexporter:
    dsn: tcp://${NOMAD_UPSTREAM_ADDR_clickhouse}/signoz_logs
    timeout: 10s
    use_new_schema: true
service:
  telemetry:
